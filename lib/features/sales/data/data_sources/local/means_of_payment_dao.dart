...
class MeansOfPaymentDao extends BaseDao<MeansOfPaymentModel> {
  ...
  Future<List<MopSelectionModel>> readByPaytypeCode(
      String payTypeCode, String edc) async {
    final result = await db.rawQuery('''
      ...
      FROM tpmt3 AS p3
      INNER JOIN (
          SELECT mopalias, bankcharge, docid, topmtId, subtype 
          FROM tpmt1
      ...
      LEFT JOIN (
          ...
          FROM tpmt4
      ) AS p4 ON p1.mopalias LIKE p4.description || '%'
      WHERE p4.description = ?
    ''', [payTypeCode, edc]);
