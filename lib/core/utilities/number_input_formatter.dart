...
class NegativeMoneyInputFormatter extends TextInputFormatter {
  @override
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
...
class MoneyInputFormatterDouble extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if (dotCounts > 1) {
      return TextEditingValue(text: oldValue.text, selection: TextSelection.collapsed(offset: oldValue.text.length));
    }

    if (newValue.text != "" && newValue.text[newValue.text.length - 1] == "." && dotCounts == 1) {
      return TextEditingValue(text: newValue.text, selection: TextSelection.collapsed(offset: newValue.text.length));
    }
    final value = Helpers.parseMoney(Helpers.revertMoneyToDecimalFormatDouble(newValue.text));
