...
abstract class ReceiptHelper {
  static ReceiptItemEntity updateReceiptItemAggregateFields(ReceiptItemEntity receiptItemEntity) {
    final newReceiptItem = receiptItemEntity.copyWith();
    final double priceQty = newReceiptItem.itemEntity.price * newReceiptItem.quantity;
    ...
    newReceiptItem.totalGross = newReceiptItem.itemEntity.includeTax == 1
        ...
        : priceQty;
    newReceiptItem.discPrctg = (newReceiptItem.discAmount ?? 0) / newReceiptItem.totalGross;
    newReceiptItem.taxAmount =
        (newReceiptItem.totalGross - (newReceiptItem.discAmount ?? 0) - (newReceiptItem.discHeaderAmount ?? 0)) *
  ...
  static ReceiptItemEntity convertItemEntityToReceiptItemEntity(ItemEntity itemEntity, double quantity) {
    return ReceiptItemEntity(
      quantity: quantity,
      totalGross: 0,
      itemEntity: itemEntity,
      taxAmount: 0,
      sellingPrice: itemEntity.includeTax == 1 ? itemEntity.price : itemEntity.dpp * ((100 + itemEntity.taxRate) / 100),
      totalAmount: 0,
      totalSellBarcode: 0,
      promos: [],
    );
  ...
  static SplitListResult<ReceiptItemEntity> splitReceiptItemEntities(
    ...
    try {
      final SplitListResult<ReceiptItemEntity> splitListResult = Helpers.splitList<ReceiptItemEntity>(
          receiptItemEntities, (ReceiptItemEntity e) => e.itemEntity.barcode == barcode);
